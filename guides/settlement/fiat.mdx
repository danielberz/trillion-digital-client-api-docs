...
```mermaid
flowchart TD
    A[Payment to instruct] --> B{Same holder,<br/>same institution?}
    ...
    D -->|No| F{Same country<br/>as the payer?}
    F -->|Yes, US| G[Fedwire same-day<br/>ACH cheaper, slower<br/>FedNow / RTP instant]
    F -->|Yes, other| H[Local domestic rail<br/>EFT · Bre-B · Fast Pay]
    F -->|No| I{Currency}
    I -->|EUR| J[SEPA]
...
| Network | Rail | Beneficiary identified by |
| --- | --- | --- |
| **Erebor Network** | `erebor` | Rail address — `@handle`, or `@routing/account` such as `@044016018/987654321` |
| **CUBIX Network** | `cubix` | Partner account UUID, plus account number |
| **FVNet** | `fvnet` | Email, plus account number and bank details |
| **Bitso** | `bitso` | Email. Mexico only |
| **BitGo GoNetwork** | `bitgo` | Go Account ID |
| **Intra-bank transfer** | `intra_bank` | Account number and email — an on-us transfer inside the beneficiary's bank |
